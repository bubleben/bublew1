<?php
    include '../connect.php';
    $sql="SELECT * FROM `dealer` ORDER BY `dea_id` ASC";
    $result=$con->query($sql);

    $date=date('d-m-Y');
    $countpro="SELECT dea_id,SUM(dea_id) as s_Total FROM dealer";
    $query = $con->query($countpro);
    $resultcountpro = mysqli_num_rows($query);

    $count="SELECT * FROM sale"; 
...
<div id="main" class="container col-xs"  onclick="window.print()">
    ...
    <h3 style="text-align: center;" > รายงานข้อมูลสมาชิกที่เป็นผู้ผลิต</h3>
        <div class="col-xs-8" style="text-align: right;">
            <p style="text-align:right;">พิมพ์วันที่&nbsp;&nbsp;:&nbsp;&nbsp;<?php echo $date;?>  </p>
            </div>
    <p>
    มีผู้จำหน่ายที่เป็นสมาชิกอยู่  &nbsp;&nbsp;<?php echo $resultcount ?> &nbsp;&nbsp;ราย
    <?php } ?>
    <table class="table"  style="width: 100%;  text-align: center;" >
        <tr>
            <th  style="width: 10%">รหัสผู้จำหน่าย</th>
            <th  style="width: 10%">ชื่อผู้จำหน่าย</th>
            <th  style="width: 10%">ชื่อแบรนด์</th>
            <th  style="width: 10%">เบอร์โทร</th>
            <th  style="width: 10%">ชื่อธนาคาร</th>
            <th  style="width: 20%">เลขที่บัญชี</th>
            <th  style="width: 20%">ชื่อเจ้าของบัญชี</th>
